...
//Publish：从注册表获取对应topic所有的Subscroiber信息，先将消息保存到map，然后异步的从map中获取消息，并逐个发送给subscriber
...
//Subscribe：将自己的ip和port以及topic,handler信息注册到注册表,当收到publish的信息后，就会触发handler
func (h *httpBroker) Subscribe(topic string, handler Handler, opts ...SubscribeOption) (Subscriber, error) {
	...
	//默认采用广播方式，当publish向该subscribe发送消息时，会遍历所有node节点，并逐个发送
	version := options.Queue
	if len(version) == 0 {
		version = broadcastVersion
	}
	//待注册的节点，包括topic,version以及node节点
